...
export function setTargetPackageForDependencies(packages: Repository, initialPackage: Package) {
...
export function setTargetPackageForAllDependencies(repository: Repository, initialPackage: Package) {
  const visited: Package[] = []
  visited.push(initialPackage)

  const targetPackages = setTargetPackageForDependencies(repository, initialPackage);

  targetPackages.filter((p) => visited.indexOf(p) === -1).forEach((p) => {
    setTargetPackageForDependencies(repository, p)
  })
}
...
setTargetPackageForAllDependencies(packages, initialPackage)
